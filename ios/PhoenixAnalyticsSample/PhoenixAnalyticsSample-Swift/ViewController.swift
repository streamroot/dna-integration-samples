...
fileprivate let ENTRY_ID = "259153"
fileprivate let ASSET_TYPE = AssetType.media
fileprivate let FORMATS = "Mobile_Devices_Main_SD"
fileprivate let PLAYBACK_CONTEXT_TYPE = PlaybackContextType.playback
...
class ViewController: UIViewController {
    ...
    fileprivate func loadStreamroot(url: URL, withId id: String) -> URL {
        guard let player = player else { return url }
        player.volume = 0
        do {
    ...
    @IBAction func changeMediaTouched(_ sender: Any) {
        ...
        player.stop() // 1. Stop Player and DNA client
        dnaClient?.stop()
        ...
        let mediaEntry = self.createMediaEntry(id: entryId,
                                               contentURL: self.loadStreamroot(url: contentURL!,
                                                                               withId: entryId))
